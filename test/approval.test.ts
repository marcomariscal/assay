...
describe(
	...
	() => {
		...
		test(
			"flags unverified spender contract",
			async () => {
				...
				expect(
					result.findings.some((finding) => finding.code === "APPROVAL_TO_UNVERIFIED"),
				).toBe(true);
			},
			120000,
		);
		...
		test(
			"flags approval to dangerous contract",
			async () => {
				...
				expect(
					result.findings.some(
						(finding) => finding.code === "APPROVAL_TO_DANGEROUS_CONTRACT",
					),
				).toBe(true);
			},
			120000,
		...
		test(
			"flags possible typosquat spender",
			async () => {
				const result = await analyzeApproval(
					{
						token: "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48",
				...
				expect(
					result.findings.some((finding) => finding.code === "POSSIBLE_TYPOSQUAT"),
				).toBe(true);
			},
			120000,
		);

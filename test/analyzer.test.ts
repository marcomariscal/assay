...
describe("analyzer", () => {
	describe("safe tokens", () => {
		test("UNI token → OK (verified, no red flags)", async () => {
			const result = await analyze("0x1f9840a85d5aF5bf1D1762F925BDADdC4201F984", "ethereum");
		...
		test("WETH on Base → OK (verified, no red flags)", async () => {
			const result = await analyze("0x4200000000000000000000000000000000000006", "base");
	...
	describe("tokens with centralization risks", () => {
		test("USDT → DANGER (mintable, blacklist, owner can drain)", async () => {
			const result = await analyze("0xdAC17F958D2ee523a2206206994597C13D831ec7", "ethereum");
	...
	describe("unverified contracts", () => {
		test("unverified contract → DANGER with UNVERIFIED finding", async () => {
			const result = await analyze("0x7768a894e6d0160530c0b386c0a963989239f107", "ethereum");
	...
	describe("proxy contracts", () => {
		test("USDC (Ethereum) → CAUTION (upgradeable proxy)", async () => {
			const result = await analyze("0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48", "ethereum");
		...
		test("USDC (Base) → CAUTION (upgradeable proxy)", async () => {
			const result = await analyze("0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913", "base");
	...
	describe("phishing labels", () => {
		test("known phishing contract → KNOWN_PHISHING finding", async () => {
			const result = await analyze("0x000011387Eb24F199e875B1325E4805EfD3b0000", "ethereum");
	...
	describe("protocol recognition", () => {
		test("Uniswap V3 router → KNOWN_PROTOCOL finding", async () => {
			const result = await analyze("0xE592427A0AEce92De3Edee1F18E0157C05861564", "ethereum");
	...
	describe("non-contracts", () => {
		test("EOA address → warning", async () => {
			...
			const result = await analyze("0x0000000000000000000000000000000000000001", "ethereum");
